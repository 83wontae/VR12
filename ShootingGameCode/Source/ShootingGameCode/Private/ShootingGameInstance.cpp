	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && UserId.IsValid())
		{
			...
			*/
			...
			SessionSettings->Set(SETTING_MAPNAME, FString("ThirdPersonMap"), EOnlineDataAdvertisementType::ViaPingOnly);
...
void UShootingGameInstance::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			{
				TArray<FBlueprintSessionResult> bpSessionResult;
				bpSessionResult.SetNum(SessionSearch->SearchResults.Num());
				...
				for (int32 SearchIdx = 0; SearchIdx < SessionSearch->SearchResults.Num(); SearchIdx++)
				{
					...
					bpSessionResult[SearchIdx].OnlineResult = SessionSearch->SearchResults[SearchIdx];
				}

				

				OnUpdateSearchResult(bpSessionResult);

				if (Fuc_Dele_SearchResult.IsBound())
					Fuc_Dele_SearchResult.Broadcast(true, bpSessionResult);
			}
		}
	}
}

bool UShootingGameInstance::MyJoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult& SearchResult)
{
	// Return bool
...
void UShootingGameInstance::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			if (PlayerController)
			{
				TravelURL = "/Game/ThirdPerson/Maps/ThirdPersonMap";
				GEngine->AddOnScreenDebugMessage(-1, 60.f, FColor::Red, FString::Printf(TEXT("ClientTravel = %s"), *TravelURL));
			...
			*/
			...
			if (PlayerController && Sessions->GetResolvedConnectString(SessionName, TravelURL))
			{
				...
				// TravelURL에서 IP 주소와 포트 번호를 추출
				FString IPAddress, strPort;
				int32 Port = 7777;
				TravelURL.Split(TEXT(":"), &IPAddress, &strPort, ESearchCase::IgnoreCase, ESearchDir::FromStart);

				// IP 주소와 포트 번호를 조합하여 새로운 형식으로 변환
				FString NewTravelURL = CombineIPAndPort(IPAddress, Port);

				GEngine->AddOnScreenDebugMessage(-1, 60.f, FColor::Red, FString::Printf(TEXT("IPAddress = %s, Port = %d"), *IPAddress, Port));

				GEngine->AddOnScreenDebugMessage(-1, 60.f, FColor::Red, FString::Printf(TEXT("NewTravelURL = %s"), *NewTravelURL));
...
void UShootingGameInstance::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			if (bWasSuccessful)
			{
				UGameplayStatics::OpenLevel(GetWorld(), "EntryLevel", true);
			}
		}
	}
}

FString UShootingGameInstance::CombineIPAndPort(const FString& IPAddress, int32 Port)
{
	...
	if (SessionSearch->SearchResults.Num() > 0)
	{
		for (int32 i = 0; i < SessionSearch->SearchResults.Num(); i++)
		{
			...
			if (SessionSearch->SearchResults[i].Session.OwningUserId != UniqueNetId)
			{
				...
				//Sessions->JoinSession(*UniqueNetId, NAME_GameSession, SearchResult);
...
void UShootingGameInstance::DestroySessionAndLeaveGame()
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			// Bind OnDestroySessionCompleteDelegate
		...
		{
			// If there's no valid session, directly call OnDestroySessionComplete
